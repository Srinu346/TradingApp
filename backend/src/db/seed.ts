...
// Helper function to generate 7-day OHLC price history
function generatePriceHistory(currentPrice: number) {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const day = new Date();
    day.setDate(day.getDate() - (6 - i)); // last 7 days
...
async function main() {
  ...
  // Shuffle stocks globally
  stocks = shuffleArray(stocks);
  ...
  console.log("✅ NSE/BSE stocks shuffled and seeded successfully!");
}
